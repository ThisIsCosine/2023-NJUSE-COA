...
public class MMU {
    ...
    private String toRealLinearAddr(String logicAddr) {
        StringBuilder segRe = new StringBuilder(logicAddr.substring(0, 16));
        segRe.insert(0, "000000000000");
        segRe.append("0000");
        StringBuilder offset = new StringBuilder(logicAddr.substring(32, 48));
        offset.insert(0, "0000000000000000");
        return new ALU().add(new DataType(segRe.toString()), new DataType(offset.toString())).toString().substring(12,32);
        // TODO
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        ...
        String base = new String(memory.getBaseOfSegDes(index));
        String segLinearAddr = new ALU().add(new DataType(base), new DataType(logicAddr.substring(16, 48))).toString();

        // TODO
        return segLinearAddr;
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        if (TLB.isAvailable) {
            TLB tlb = TLB.getTLB();
            if (tlb.inTLB(vIndex))
                return new String(tlb.getFrameOfPage(vIndex)) + linearAddr.substring(20, 32);
            else {
                String pIndex = new String(memory.getFrameOfPage(vIndex));
                return pIndex + linearAddr.substring(20, 32);
            }
        }
        else {
            String pIndex = new String(memory.getFrameOfPage(vIndex));
